...
document.getElementById('editProductForm').addEventListener('submit', function(e) {
    ...
    // فحص Uniqueness للباركود (تجاهل المنتج الحالي)
    const barcode = document.getElementById('editProductBarcode').value.trim();
    if (barcode) {
        const cleanBarcode = barcode.replace(/[^\d]/g, '');
        const existingProduct = products.find(p => p.barcode === cleanBarcode && p.id !== editId);
        if (existingProduct) {
            showMessage(`❌ الباركود ${cleanBarcode} مستخدم بالفعل في منتج: ${existingProduct.name}`, 'error');
            return;
        }
    }
...
function applySalesFilter() {
    ...
    showMessage(`تم تطبيق التصفية: ${totalTransactions} معاملة`, 'success');
}
...
function searchByBarcode(barcode) {
    ...
    // تنظيف الباركود: حذف المسافات، تحويل إلى أرقام إنجليزية فقط
    const cleanBarcode = barcode.toString().trim().replace(/[^\d]/g, '');
    console.log('الباركود المنظف:', cleanBarcode); // للتشخيص
    
    // التحقق من صحة الباركود المنظف
    if (!cleanBarcode || cleanBarcode.length === 0) {
        console.log('=== باركود فارغ أو غير صالح ===');
        showMessage('⚠️ باركود غير صالح', 'error');
        isProcessingBarcode = false;
        lastProcessedBarcode = '';
        return;
    }
    
    // البحث بالباركود المطابق تماماً فقط (مطابقة تامة 100%)
    ...
    if (product) {
        ...
        // رسالة خطأ واضحة - لا يوجد منتج بهذا الباركود
        showMessage('❌ لا يوجد منتج بهذا الباركود', 'error');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    e.preventDefault();
            ...
            // فحص Uniqueness للباركود
            const barcode = document.getElementById('productBarcode').value.trim();
            if (barcode) {
                const cleanBarcode = barcode.replace(/[^\d]/g, '');
                const existingProduct = products.find(p => p.barcode === cleanBarcode);
                if (existingProduct) {
                    showMessage(`❌ الباركود ${cleanBarcode} مستخدم بالفعل في منتج: ${existingProduct.name}`, 'error');
                    return;
                }
